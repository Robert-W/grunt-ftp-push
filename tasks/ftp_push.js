...
module.exports = function (grunt) {
    'use strict';
    var Ftp = require('jsftp-rmr')(require('jsftp')),
        rootDestination,
        ftpServer,
        options,
        done;
    ...
    grunt.registerMultiTask('ftp_push', 'Deploy files to a FTP server.', function () {
        ...
        ftpServer.auth(credentials.username, credentials.password, function (err, res) {
            ...
            // Clean destination directory if required
            if (options.clean === true) {
                ftpServer.rmr(options.dest, function (error) {
                    if (error) {
                        grunt.fail.warn(error);
                    } else {
                        grunt.log.ok(options.dest + " directory successfully deleted!");
                        createDirectoriesAndUploadFiles();
                    }
                });
            } else {
                createDirectoriesAndUploadFiles();
            ...
            function createDirectoriesAndUploadFiles() {
